...
import simpledb.transaction.WaitForGraph;
...
public class BufferPool {
    ...
    public boolean holdsLock(TransactionId tid, PageId pid) {
        synchronized (this) {
            return lockManager.hasLockOn(tid, pid);
        }
    }
    ...
    public void transactionComplete(TransactionId tid, boolean commit) {
        synchronized (this) {
            if (!commit) {
                ArrayList<PageId> list = lockManager.releaseAllLock(tid);
                for (PageId pid : list)
                    pages.put(pid, Database.getCatalog()
